...
export function getLocalDeploymentParams(): {
  ...
  dotenv.config()
  if (!process.env.LOCAL_GRAPH_NODE_URL || !process.env.LOCAL_IPFS_URL) {
    throw new Error('LOCAL_GRAPH_NODE_URL and LOCAL_IPFS_URL must be set')
  }
  return {
    node: process.env.LOCAL_GRAPH_NODE_URL,
    ipfs: process.env.LOCAL_IPFS_URL,
  }
}
