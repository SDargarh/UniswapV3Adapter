...
import { getAlchemyDeploymentParams, getSubgraphName, prepare, getLocalDeploymentParams } from './prepareNetwork'
...
//   }
  ...
  if (isLocal) {
    ...
    graphDeployCommand = `graph deploy --node ${node} --ipfs ${ipfs} --version-label ${gitHashString} ${subgraphName} ${subgraphType}-subgraph.yaml`
  } else {
    
    const { node, ipfs, deployKey } = getAlchemyDeploymentParams()
    
    graphDeployCommand = `graph deploy --node ${node} --ipfs ${ipfs} --deploy-key ${deployKey} --version-label ${gitHashString} ${subgraphName} ${subgraphType}-subgraph.yaml`
  }

  try {
    
    console.log(`Running command: ${graphDeployCommand}`)
    const { stdout, stderr } = await exec(graphDeployCommand)
